...
test("an owner can send a stored invoice, and client-supplied invoice details cannot change its email", async () => {
  ...
  try {
    ...
    assert.equal(email.subject, "Stored company sent you a bill for Stored invoice title");
    assert.match(email.html, /0\.0025 ETH/);
    assert.match(email.html, /Stored customer/);
    assert.match(email.html, /Stored invoice title/);
    assert.match(email.html, /Stored company sent you a bill for Stored invoice title/);
    assert.match(email.html, /View and pay/);
    assert.match(email.html, /Payment is due on February 29, 2028/);
    ...
    assert.match(email.text, /Stored company sent you a bill for Stored invoice title/);
    ...
    assert.ok(email.text.includes(`View and pay: ${result.paymentLink}`));
