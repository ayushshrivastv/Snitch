...
- `/workspace`: authenticated workspace. The same Snitchpay.co showcase appears alongside the user's company accounts, while the sidebar keeps the authenticated profile name. `Create Account` creates a company and its own Privy wallet. The Snitchpay.co wallet can also be connected explicitly: this creates a separate owner-scoped company record and wallet, not a treasury shared with other visitors. Company records and public treasury identifiers persist in SQLite. Displayed team roles do not confer signing authority.
